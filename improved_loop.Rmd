...
mys_plot <- ggplot() +
   ...
   scale_x_date(date_labels = "%Y",
                date_breaks = "1 year") +
   ggtitle("Malaysia's Fishing Effort from 2017-2019") +
   xlab("Year") + 
   ylab("Mean Fishing Hours") +
   theme(panel.background = element_blank(),
         axis.title.x = element_text(color = "black", size = 15),
...
ggsave(filename = "malaysia_fishing_effort.png", plot = mys_plot, path = here(), width = 12,
  height = 7)
```
...
for (i in 1:length(models)) {
  predicted_effort_2020 <- penguin$X.Intercept.[i] + penguin$year.1[i]
