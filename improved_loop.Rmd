...
comparison_2020_pos <- comparison_2020_pos %>% 
  ...
  mutate(change_direc = case_when(
    difference > 0 ~ "fished_less_than_trend",
    difference < 0 ~ "fished_more_than_trend"))
```
...
#   ) %>%
#   fmt_passthrough(
#     columns = vars(unique(bad_countries$flag_gfw))
#   ) %>% 
#   fmt_number(
#     columns = vars(mean),
#     decimals = 4
...
#   ) %>%
#   fmt_number(
#     columns = vars(seventy_fifth_percentile),
#     decimals = 4
#   ) %>%
#   fmt_number(
#     columns = vars(standard_deviation),
#     decimals = 4
#   ) %>%
#   cols_label(quantile = "Quantile" , mean = "Mean", median = "Median", seventy_fifth_percentile = "75th Percentile", standard_deviation = "Standard Deviation")
```
